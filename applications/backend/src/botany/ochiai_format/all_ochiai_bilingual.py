...
from src.botany.ochiai_format.format_template_en import (
...
with get_openai_callback() as cb:
    ...
    for i, (save_name, query, template) in enumerate(
        ...
        with open(
            f"src/botany/ochiai_format/bilingual_{paper_acronym}_{i+3}_{save_name}.txt",
            "w",
        ) as f:
            f.write(output)
    ...
    # Save the result to a txt file
    with open(
        f"src/botany/ochiai_format/bilingual_{paper_acronym}_2_contribution.txt", "w"
    ) as f:
        f.write(output)
    ...
    # Save the result to a txt file
    with open(
        f"src/botany/ochiai_format/bilingual_{paper_acronym}_1_abstract.txt", "w"
    ) as f:
        f.write(output)
