...
with get_openai_callback() as cb:
    ...
    for i, (save_name, query, template) in enumerate(zip(
        ...
        with open(f"src/botany/ochiai_format/bilingual_{paper_acronym}_{i+3}_{save_name}.txt", "w") as f:
            f.write(output)
    ...
    contribution_template = """Write a 50-word summary of the contribution of this study from the following statement IN JAPANESE:
    ...
    Output IN JAPANESE:"""
    problems_template = """Write a 200-letter summary of the problems with the privious studies from the following statement IN JAPANESE:
    ...
    combine_template = """Write a 50-word summary of what makes this study superior to previous studies based on the given <CONTRIBUTION OF THIS STUDY> and <PROBLEMS OF PREVIOUS STUDIES> IN JAPANESE:
    ...
    # Save the result to a txt file
    with open(f"src/botany/ochiai_format/bilingual_{paper_acronym}_2_contribution.txt", "w") as f:
        f.write(output)
    ...
    bilingual_prompt = PromptTemplate(template=prompt_template, input_variables=["text"])
    # Summarize a document chunk-wise first, then summarize those summaries in a single summary
    chain = load_summarize_chain(
        llm=chat_model,
        map_prompt = bilingual_prompt,
        combine_prompt = bilingual_prompt,
    ...
    # Save the result to a txt file
    with open(f"src/botany/ochiai_format/bilingual_{paper_acronym}_1_abstract.txt", "w") as f:
        f.write(output)
